...
public class QrVerificationService {
	...
	@Transactional(readOnly = true) public QrCodeResponse get(UUID employeeId) { requireEmployeeAccess(employeeId); var actor=currentUser.requireCurrentUser(); EmployeeQrCode c=codes.findByOrganizationIdAndEmployeeIdAndStatus(actor.organizationId(), employeeId, EmployeeQrCode.Status.ACTIVE).orElseThrow(() -> new ResourceNotFoundException("O colaborador não possui QR Code ativo.")); return response(c, decrypt(c.getTokenCiphertext())); }
	...
	@Transactional public QrCodeResponse generate(UUID employeeId) { requireAdmin(); var actor=currentUser.requireCurrentUser(); EmployeeQrCode active=codes.findByOrganizationIdAndEmployeeIdAndStatus(actor.organizationId(), employeeId, EmployeeQrCode.Status.ACTIVE).orElse(null); if (active != null) { active.revoke(actor.userId(), "substituído", clock.instant()); codes.flush(); } String token=randomToken(); Instant now=clock.instant(); EmployeeQrCode c=codes.save(new EmployeeQrCode(actor.organizationId(), employeeId, hash(token), encrypt(token), actor.userId(), now)); audit.record(new AuditPort.AuditRecord(actor.organizationId(), actor.userId(), "QR_CODE_GENERATED", "EMPLOYEE_QR_CODE", c.getId(), now, java.util.Map.of("employeeId", employeeId.toString()))); return response(c, token); }
	@Transactional public QrCodeResponse revoke(UUID employeeId, String reason) { requireAdmin(); var actor=currentUser.requireCurrentUser(); EmployeeQrCode c=codes.findByOrganizationIdAndEmployeeIdAndStatus(actor.organizationId(), employeeId, EmployeeQrCode.Status.ACTIVE).orElseThrow(() -> new ResourceNotFoundException("O colaborador não possui QR Code ativo.")); c.revoke(actor.userId(), reason, clock.instant()); audit.record(new AuditPort.AuditRecord(actor.organizationId(), actor.userId(), "QR_CODE_REVOKED", "EMPLOYEE_QR_CODE", c.getId(), clock.instant(), java.util.Map.of("reason", reason))); return response(c, null); }
	...
	@Transactional public QrVerificationResponse verify(String token) { String tokenHash=hash(token); EmployeeQrCode c=codes.findByTokenHash(tokenHash).orElse(null); var user=currentUser.requireCurrentUser(); if (c == null || !user.organizationId().equals(c.getOrganizationId())) { log(user.organizationId(),null,user.userId(),"UNKNOWN",tokenHash); throw new ResourceNotFoundException("O QR Code não existe."); } if (c.getStatus() != EmployeeQrCode.Status.ACTIVE) { log(user.organizationId(),c,user.userId(),"REVOKED",tokenHash); throw new ResourceConflictException("QR_CODE_REVOKED", "O QR Code foi revogado."); } requireEmployeeAccess(c.getEmployeeId()); log(user.organizationId(),c,user.userId(),"VALID",tokenHash); var employee=employees.requireEmployee(c.getEmployeeId()); var values=completions.findAllByOrganizationIdAndEmployeeIdOrderByCompletedAtDescIdDesc(user.organizationId(), c.getEmployeeId()); var trainingValues=values.stream().map(v -> { var t=trainings.summary(v.getTrainingId()); return new QrVerificationResponse.Training(t.name(), t.code(), t.regulatoryStandard(), v.getCompletedAt(), v.getExpirationDate(), "COMPLETED"); }).toList(); var activityValues=qualifications.list(c.getEmployeeId(), null, null, org.springframework.data.domain.Pageable.unpaged()).getContent().stream().map(q -> new QrVerificationResponse.Activity(q.activity().name(), q.status().name(), q.blockingReasons().stream().map(dev.igorbarbosa.worktrainingsystem.qualifications.api.QualificationResponse.BlockingReason::trainingName).toList())).toList(); return new QrVerificationResponse(new QrVerificationResponse.Employee(employee.name(), employee.registration(), employee.jobId().toString()), trainingValues, trainingValues.stream().filter(QrVerificationResponse.Training::isRegulatoryStandard).toList(), activityValues); }
	@Transactional(readOnly = true) public Page<QrAccessLogResponse> logs(UUID codeId, Pageable pageable) { requireAdmin(); var actor=currentUser.requireCurrentUser(); return logs.findAllByOrganizationIdAndQrCodeIdOrderByQueriedAtDesc(actor.organizationId(), codeId, pageable).map(l -> new QrAccessLogResponse(l.getId(), l.getQrCodeId(), l.getQueriedByUserId(), l.getQueriedAt(), l.getResult(), l.getRequestId())); }
	private void log(UUID organizationId, EmployeeQrCode c, UUID user, String result, String hash) { logs.save(new QrCodeAccessLog(organizationId, c == null ? null : c.getId(), user, clock.instant(), result, UUID.randomUUID().toString(), hash)); }
