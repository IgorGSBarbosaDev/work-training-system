...
public class QrVerificationService {
	...
	private final AuditPort audit; private final JwtProperties jwt; private final Clock clock; private final SecureRandom random = new SecureRandom();
	public QrVerificationService(EmployeeQrCodeRepository codes, QrCodeAccessLogRepository logs, TrainingCompletionRepository completions,
			...
			AuditPort audit, JwtProperties jwt, Clock clock) { this.codes=codes; this.logs=logs; this.completions=completions; this.employees=employees; this.trainings=trainings; this.authorization=authorization; this.currentUser=currentUser; this.audit=audit; this.jwt=jwt; this.clock=clock; }
	...
	@Transactional public QrCodeResponse generate(UUID employeeId) { requireAdmin(); codes.findByOrganizationIdAndEmployeeIdAndStatus(DEFAULT_ORGANIZATION_ID, employeeId, EmployeeQrCode.Status.ACTIVE).ifPresent(c -> c.revoke(currentUser.requireCurrentUser().userId(), "substituído", clock.instant())); String token=randomToken(); Instant now=clock.instant(); EmployeeQrCode c=codes.save(new EmployeeQrCode(DEFAULT_ORGANIZATION_ID, employeeId, hash(token), encrypt(token), currentUser.requireCurrentUser().userId(), now)); audit.record(new AuditPort.AuditRecord(currentUser.requireCurrentUser().userId(), "QR_CODE_GENERATED", "EMPLOYEE_QR_CODE", c.getId(), now, java.util.Map.of("employeeId", employeeId.toString()))); return response(c, token); }
	...
	private QrCodeResponse response(EmployeeQrCode c, String token) { return new QrCodeResponse(c.getId(), c.getEmployeeId(), token, c.getStatus(), c.getGeneratedAt(), c.getRevokedAt(), c.getRevocationReason()); }
