...
| Frontend | `npm ci`, `npm test`, `npm run lint`, `npm run build` | Node 22 | Suíte, lint e build verdes | Validado localmente; imagem Docker compilada com Node 22 | 29/08/2026 | working tree | Job frontend |
...
| Fluxo API crítico | `node scripts/acceptance/smoke.mjs` | Seed + Compose | Fluxo treinamento → vídeo → avaliação → conclusão → certificado | Validado: 11 checks; resultados `FAILED` e `APPROVED` | 29/08/2026 | working tree | `smoke-result.json` |
...
| Browser acceptance | `npm run e2e` | Frontend, seed e Chromium | Login, escopos e telas integradas | Validado: 5 testes | 29/08/2026 | working tree | Screenshots/traces somente em falha |
